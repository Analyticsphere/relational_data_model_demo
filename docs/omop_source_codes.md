...
*Design note for `sql/omop/response_source_codes.sql` and `sql/omop/response_unique_id_udf.sql`.
...
> **Everything else in `response_source_codes` is *decoration*** — `response_kind`, `value_norm`,
> `source_code_description`. Useful, but they **never affect `response_unique_id`**, so they can be
> changed freely.
...
- `source_code_description` = `source_code_description` (coded → question text + answer label from
  the dictionary; free text → question text + the verbatim string)
- optionally carry `question_concept_id` / `response_value_verbatim` so mappings round-trip back to
  Connect.
...
Substitute `${PROJECT}`, ensure the `response_unique_id` UDF is deployed
(`sql/omop/response_unique_id_udf.sql`), and ensure `relational.responses` + the `question` /
`response` / `question_response` dims are loaded, then run `sql/omop/response_source_codes.sql`.
The output is deterministic — re-running reproduces identical `response_unique_id` values.
