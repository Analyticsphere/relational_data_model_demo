...
WITH pivoted AS (
  SELECT
    ...
    MAX(IF(question_concept_id = '367803647'
           AND source_question_concept_id = '367803647', response_value_as_string, NULL)) AS education_concept_id,
...
responses |>
  ...
  count(education = format_value, sort = TRUE)
```
```python
# python
(responses
   ...
   .group_by(education=_.format_value)
   .aggregate(n=_.count())
   .order_by(_.n.desc()))
```
...
```sql
-- sql
SELECT question_concept_id, question_text, response_concept_id, format_value
