...
::: {.callout-note}
**Rendering this document executes every query against the stage project.** It needs a live `bigrquery`
connection (authenticate with `bigrquery::bq_auth()`) and the R packages `DBI`, `dplyr`, `dbplyr`,
`bigrquery`. Each example is shown two ways — an executable **`{sql}`** chunk (knitr runs it against `con`)
and an executable **`{r}`** chunk (dbplyr, lazy). To render a static copy without touching BigQuery, set
`eval: false` in the YAML header.
:::
...
The `{sql}` chunks below run against `con`; the `{r}` chunks use lazy `dbplyr` handles (nothing is pulled
until the tibble prints or you `collect()`).
...
v_resp_enriched |>
  ...
  count(answer = response_label, sort = TRUE)     # lazy — prints a preview; add |> collect() to pull all
```
