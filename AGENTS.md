...
A normalized relational (star-schema) data model for the Connect cohort, replacing the wide
"dancing-schema" survey tables, feeding the eventual researcher-facing **PR2** warehouse. **The model is
accepted** (the Dictionary-Direct model — see `docs/enhancement_backlog.md`). Active work is now
**operational documentation**: the enhancement backlog + a set of grounded **investigation surveys**
(`docs/*_survey.md`, `docs/value_typing.md`) that scope each enhancement, plus a stage proof-of-concept
pipeline (`sql/unpivot_stage/`, `scripts/setup_relational.py`). The original kick-off **pitch + slide deck**
are archived under `docs/archive/` (decision made; kept for reference).
...
| **`docs/enhancement_backlog.md`** | **start here** — the accepted-model decision + each candidate enhancement (value, sketch, cost) + shared dependencies |
| **`docs/*_survey.md` · `docs/value_typing.md`** | **the investigation surveys** — grounded, quantitative scoping per enhancement: question types, value typing, version handling, skip logic, session derivation, governance, Quest↔concept-ID linkage |
...
| `docs/*.svg` · `docs/cid_tool_erd.drawio.png` | model ERDs (Dictionary-Direct + event plane) + the CIDTool ERD |
| `docs/archive/` | **archived kick-off pitch** — `internal_pitch.md`, the slide deck (`.pptx`) + `build_deck.py`, `slide_assets/` (decision made; reference only) |
...
- **The kick-off deck is archived** under `docs/archive/` (decision made). It's still regenerable from
  `docs/archive/build_deck.py` (edit the script, not the `.pptx`) if the pitch is revisited.
