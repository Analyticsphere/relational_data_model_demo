...
--
-- Pattern: the four base recodes are pure code->label (labels JOIN the dictionary `response` dim). The two
-- DERIVED variables (ever_smoker_override, cigarette_cats) are genuine analytic logic — kept as CASE and
-- evaluated on the coded concept IDs (not label strings) so they can't drift with dictionary wording.
-- Column order pairs each coded answer with its label: <x>_concept_id, <x>, ...  (derived vars last).
--
...
CREATE OR REPLACE VIEW `${PROJECT}.relational.mart_smoking`(
  ...
  cigs_lifetime_concept_id  OPTIONS(description="Lifetime cigarettes answer concept ID (D_763164658)"),
  cigs_lifetime             OPTIONS(description="Lifetime cigarettes label from the dictionary response dim"),
  smoke_cigs_now_concept_id OPTIONS(description="Smokes cigarettes now answer concept ID (D_639684251)"),
  smoke_cigs_now            OPTIONS(description="Smokes now label from the dictionary response dim"),
  cigs_lasttime_concept_id  OPTIONS(description="Last time smoked answer concept ID (D_798549704)"),
...
SELECT
  ...
  CASE
    WHEN p.cigs_lifetime_concept_id = '132232896' THEN 1
    WHEN p.cigs_lifetime_concept_id IN ('151488193','805449318','486319890') THEN 0
    ELSE NULL
  END AS ever_smoker_override,

  -- cigarette_cats (derived Never/Current/Former collapse), evaluated on concept IDs
  CASE
    WHEN p.smoker_status_concept_id = '104430631' THEN 'Never Smoker'                                    -- No
    WHEN p.smoker_status_concept_id = '353358909'
         AND p.cigs_lifetime_concept_id IN ('151488193','805449318') THEN 'Never Smoker'                 -- Yes but <50 lifetime
    WHEN p.smoker_status_concept_id = '353358909' AND p.cigs_lifetime_concept_id IN ('486319890','132232896')
         AND p.cigs_lasttime_concept_id IN ('317567178','484055234') THEN 'Current Smoker'               -- smoked within past year
    WHEN p.smoker_status_concept_id = '353358909' AND p.cigs_lifetime_concept_id IN ('486319890','132232896')
         AND p.cigs_lasttime_concept_id = '802197176' THEN 'Former Smoker'                               -- last >1yr ago
    WHEN p.smoker_status_concept_id = '353358909' AND p.cigs_lifetime_concept_id IN ('486319890','132232896')
         AND p.cigs_lasttime_concept_id IS NULL
         AND p.smoke_cigs_now_concept_id IN ('299561721','716761013','804785430') THEN 'Current Smoker'  -- no lasttime, smokes now
    WHEN p.smoker_status_concept_id = '353358909' AND p.cigs_lifetime_concept_id IN ('486319890','132232896')
         AND p.cigs_lasttime_concept_id IS NULL
         AND p.smoke_cigs_now_concept_id = '419415087' THEN 'Former Smoker'                              -- no lasttime, not now
    WHEN p.smoker_status_concept_id = '353358909' AND p.cigs_lifetime_concept_id IS NULL
         AND p.smoke_cigs_now_concept_id = '419415087' AND p.cigs_lasttime_concept_id = '802197176' THEN 'Former Smoker'
